...
## -*- texinfo -*- 
## @deftypefn {} {@var{retval} =} EncodeImageLZ77_Re (@var{input1}, @var{input2})
##
## @seealso{}
## @end deftypefn
...
function Stream = EncodeImageLZ77_Re (Image,Nw,Mw)
  %LZ77 Example
  ...
  [Symb,ImIndex] = ImageSymbolsIndexRe(Image);
  ...
  while j <= Max_j  %Percorremos todos os simbolos do buffer (Dimensao da imagem = N*M);
    ...
    while FoundFlag && ObsWndNotOver
      ...
      i = FindSequenceInWindowRe(SearchStr',SlidingWnd'); %Vamos ver se temos aquela sequencia na janela deslizante
